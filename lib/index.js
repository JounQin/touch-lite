...
;['Array', 'Function', 'Object', 'String'].forEach(type => (utils[`is${type}`] = trueTypeFunc(type)))
...
const EVENTS = ['start', 'moveStart', 'moving', 'moveEnd', 'end', 'tab', 'dblTap', 'mltTap',
...
export default class Touch {
  ...
  _start() {
    ...
    on(this.el, EVENT.start, this._eStart = e => {
      console.log('xxxxxx')
      ...
      e = this.actualEvent(e, !isMouseDown)
  ...
  _move() {
    on(this.el, EVENT.move, this._eMove = e => {
      ...
      this._isPrevent('moving', extend(e, {changedX, changedY}))
    })
  }

  _end() {
    on(this.el, EVENT.end, this._eEnd = e => {
      if (e.type === MOUSE_UP) {
        off(document, {
          [MOUSE_MOVE]: this._eMove,
          [MOUSE_UP]: this._eEnd
      ...
      this._timeout = setTimeout(() => {
        ...
        } else if (this._isPrevent('mltTap', extend(e, {tapped}))) return
